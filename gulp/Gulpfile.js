...
gulp.task('scripts_files', function() {
  return gulp
    .src([config.src + 'js/scripts/*.js'], { base: config.src })
    .pipe(gulp.dest(config.dest));
});
...
// Move non compressed files
gulp.task('move_images_non_compressed', function() {
  return gulp
    .src(
      [
        config.src + 'img/**/*.*',
...
gulp.task('images', function() {
  return gulp
    .src([config.src + 'img/**/*'], { base: config.src })
    .pipe(gulp.dest(config.dest));
});
// Comprime las imagenes
gulp.task('images_min', function(callback) {
  runSequence('png', 'svg', 'move_images_non_compressed', callback);
});
...
gulp.task('watch', ['browserSync'], function() {
  ...
  gulp.watch(config.src + 'js/vendor/**/*', ['copy:vendor']);
  ...
  gulp.watch('config.json', function() {
    runSequence('loadConfig', 'scripts', 'sass', 'copy:fonts', 'copy:vendor');
  });
});
